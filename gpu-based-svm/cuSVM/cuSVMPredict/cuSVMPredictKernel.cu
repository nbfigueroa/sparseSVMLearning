...
//The memory access pattern and structure of this code is derived from Vasily Volkov's highly optimized matrix-matrix multiply CUDA code.
...
{	
    ...
    float* d_PaddedSvs;
    mxCUDA_SAFE_CALL( cudaMalloc( (void**) &d_PaddedSvs, paddedn*paddedk*sizeof(float)));

    mxCUDA_SAFE_CALL( cudaMemset(d_PaddedSvs,0.f,paddedn*paddedk*sizeof(float)));
    mxCUDA_SAFE_CALL( cudaMemcpy(d_PaddedSvs, Svs, sizeof(float)*n*k,cudaMemcpyHostToDevice));
  
    float* d_PaddedSvsT;
    mxCUDA_SAFE_CALL( cudaMalloc( (void**) &d_PaddedSvsT, paddedn*paddedk*sizeof(float)));
  
    mxCUDA_SAFE_CALL(  cudaMemset(d_PaddedSvsT,0.f,paddedn*paddedk*sizeof(float)));
    ...
    float * d_TestInter;
    float * d_QInter;
    mxCUDA_SAFE_CALL( cudaMalloc( (void**) &d_TestInter, RowsPerIter*paddedk*sizeof(float)));
   
    mxCUDA_SAFE_CALL(cudaMemset(d_TestInter,0.f,RowsPerIter*paddedk*sizeof(float)));
    mxCUDA_SAFE_CALL( cudaMalloc( (void**) &d_QInter, RowsPerIter*paddedn*sizeof(float)));
    ...
    mxCUDA_SAFE_CALL( cudaMalloc( (void**) &d_prediction, NecIterations*RowsPerIter*sizeof(float)));
